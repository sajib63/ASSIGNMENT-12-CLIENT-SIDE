...
const Mercedes = () => {
    ...
    const {data: mercedesCars=[], isLoading}=useQuery({
        queryKey:['mercedes'],
        queryFn: async ()=>{
            const res= await fetch('https://y-sajib63.vercel.app/mercedes')
    ...
    return (
        <div>
            ...
            {
                mercedesCars?.map(cars=> <AllCarsCard key={cars._id} cars={cars} setModalData={setModalData} setReport={setReport}></AllCarsCard>)
            }
           </div>
